...
def stream(video_path):
    ...
    fr = FaceRecognition(r"Facenet_masked_model.pb", r"Taylor")
                ...
                if len(bboxes) > 0:
                    for i, bbox in enumerate(bboxes):
                        ...
                        if name != "Unknown":
                            ...
                            save_path = f"{name}_{time_seconds}s_{save_count}.jpg"
                            save_path = os.path.join(r"person_found", save_path)
...
video_path = "Taylor.mp4"  # Replace with the actual path to your video file
stream(video_path)
