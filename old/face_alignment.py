...
def model_restore_from_pb(pb_path, node_dict, GPU_ratio=None):
    with tf.Graph().as_default():
        ...
        if GPU_ratio is None:
            # The program can access as much resource as possible
...
class FaceMaskDetection():
    def __init__(self, pb_path, margin=44, GPU_ratio=0.1):
        ...
        sess, tf_dict = model_restore_from_pb(
            pb_path, node_dict, GPU_ratio=GPU_ratio)
    ...
    def inference(self, img_4d, ori_height, ori_width):
        ...
        for idx in keep_idxs:
            ...
            ymax = np.minimum(bbox[3] * ori_height +
                              self.margin / 2, ori_height)
